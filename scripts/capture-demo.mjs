...
const port = Number.parseInt(process.env.ASTROGUIDE_CAPTURE_PORT || '2513', 10);
const baseURL = `http://127.0.0.1:${port}`;
const captureURL = `${baseURL}/?lang=en`;
const viteBin = path.join(projectDirectory, 'node_modules', 'vite', 'bin', 'vite.js');

if (!Number.isInteger(port) || port < 1 || port > 65_535) {
  throw new Error(`Invalid capture port: ${process.env.ASTROGUIDE_CAPTURE_PORT}`);
}
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for the AstroGuide demo capture server.');
}
...
function installCaptureHooks(page) {
  return page.addInitScript(() => {
    try {
      ...
      localStorage.setItem('astroguide.ui_language', 'en');
    } catch {}
  });
}
...
async function capture(page, name) {
  ...
  console.log(`[ASTROGUIDE] Captured ${path.relative(projectDirectory, destination)}`);
}
...
try {
  ...
  await page.getByRole('button', { name: '3D exploration' }).waitFor({ timeout: 30_000 });
  await dismissDemoChrome(page);
  await page.getByRole('button', { name: /Saturn/ }).first().click();
  await page.locator('canvas').first().waitFor({ timeout: 20_000 });
  await page.waitForTimeout(1_200);
  await capture(page, 'astroguide-demo-3d');

  await page.getByRole('button', { name: 'Map' }).click();
  await page.getByLabel('Illustrative map of celestial objects').waitFor({ timeout: 20_000 });
  await page.waitForTimeout(600);
  await capture(page, 'astroguide-demo-map');

  await page.getByRole('button', { name: 'Comparison' }).click();
  await page.getByRole('heading', { name: 'Size comparison' }).waitFor({ timeout: 20_000 });
  await capture(page, 'astroguide-demo-comparison');
  await desktop.close();
} finally {
  await browser?.close();
  await stopServer();
